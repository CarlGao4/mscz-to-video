...
class MainWindow(QWidget):
    ...
    def search_devices(self):
        ...
        if (
            ...
            and "intel_extension_for_pytorch" in sys.modules
            and ipex.xpu.device_count()
        ):
            global find_device_win
            import find_device_win

            self.exec_in_main(self.ask_AOT)

    def ask_AOT(self):
        if find_device_win.ipex_version_available(ipex.__version__) and os.path.getsize(ipex.dlls[0]) < 1000000000:
            intel_gpus = []
            for gpu in find_device_win.gpus:
                if gpu_ver := find_device_win.is_intel_supported(gpu[1], gpu[2], ipex.__version__):
                    intel_gpus.append((gpu[0], gpu_ver))
            if intel_gpus:
                prompt = "Detected Intel GPU support, but AOT is not enabled.\n\n"
            intel_gpu_links = {}
            for i in intel_gpus:
                if link := find_device_win.get_download_link(i[1], ipex.__version__):
                    intel_gpu_links.setdefault(link, []).append(i[1])
            if len(intel_gpu_links) == 1:
                m = QMessageBox(self)
                m.setWindowTitle("AOT download")
                prompt += "Found supported Intel GPU(s):\n"
                ...
                while True:
                    ...
                    if m.clickedButton() == download_button:
                        ...
                        webbrowser.open("https://github.com/CarlGao4/mscz-to-video/blob/main/MKL-AOT.md")
                    else:
                        break
            elif len(intel_gpus) > 1:
                m = QMessageBox(self)
                    ...
                    if m.clickedButton() == doc_button:
                        ...
                        for i in download_buttons:
                            if m.clickedButton() == i[1]:
                                webbrowser.open(i[0], ipex.__version__)
                                break
